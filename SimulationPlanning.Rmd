...
for (i in 1:length(mFemale))
{
  ...
  write_full_param(homedir = "Simulations/LongTry", focaldir = paste0("LongTry_", i),
                   ...
                   WritePeriod=100)
  qsub_gadi(jobdir =paste0("Simulations/LongTry/LongTry_",i), gsubfile = paste0("gadi_LongTry_",i))
}
...
```{r}
introgprofile <- read.table(file = "Simulations/test/IntrogProfile2.txt", header = T)
habitat2 <- introgprofile %>% filter(x>1)
...
```{r}
introgprofile <- read.table(file = "Simulations/test/IntrogProfile4.txt", header = T)
habitat2 <- introgprofile %>% filter(x>1)
habitat2$MeanA <- apply(habitat2 %>% select(num_range("Locus", 0:19)), 1, mean)
meltedleadnu <- pivot_longer(habitat2, cols = c(LocusMt, MeanA, LocusAdapt0),
                           names_to = "marker",
...
plot(habitat2$MeanA, habitat2$Year)
...
ggplot(habitat2, aes(x=x, y=MeanA-LocusMt, colour=as.factor(Run))) + 
  ...
  labs(title="Discordance")+ylim(-1,1)+ labs(title = 'Year: {frame_time}')+ 
